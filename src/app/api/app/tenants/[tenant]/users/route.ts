...
import { requireTenantContextApi, canManageUsers } from "@/lib/tenant-auth";
import { z } from "zod";
...
export async function PATCH(req: Request, { params }: { params: { tenant: string } }) {
  const ip = getClientIp(req);
  ...
  if (myRole === UserRole.ADMIN) {
    if (membership.role === UserRole.OWNER) return jsonError("cannot_modify_owner", 403);
    if (newRole === UserRole.OWNER) return jsonError("forbidden_role", 403);
  }
  ...
  // Prevent changing your own role unless OWNER
  if (membership.userId === me.id && myRole !== UserRole.OWNER && newRole !== myRole) {
...
export async function DELETE(req: Request, { params }: { params: { tenant: string } }) {
  const ip = getClientIp(req);
  ...
  if (myRole === UserRole.ADMIN && membership.role === UserRole.OWNER) {
    return jsonError("cannot_remove_owner", 403);
  }
  ...
  if (membership.userId === me.id && myRole !== UserRole.OWNER) {
